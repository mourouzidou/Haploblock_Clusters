...
- [ ] Figure out way to look at cancer drivers (Michael and Elena)
      Variants that are characterized as cancer driver mutations are the ones that are responsible for turning normal cells to cancer cells. Such mutations are are typically located in genes associated with functions such as apoptosis, proliferation, DNA repair and other critical cellular processes. Databases such as COSMIC (Catalogue Of Somatic Mutations In Cancer), OncoKB, or IntOGen store information on cancer driver mutations and most of them provide such data in a .vcf format. 
We can use `bcftools` to filter cancer driver mutations out of the initial vcf data and VEP (Variant Effect Predictor) to annotate them with functional details and potentially determine the affected pathways and therapeutic implications.
